...
The reason the $\textit{maxLineGap}$ and the $\textit{minLineLength}$ are relatively low w.r.t. the resolution is because the cost matrix nudges the variables towards a safe value. It rather sees a landmark undetected than a misclassified landmark, which -as explained in the previous section- is prefered. 
