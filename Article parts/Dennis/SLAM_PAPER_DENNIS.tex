...
\appendix
\section{EKF SLAM Matrices}
This appendix shows all the matrices used by the EKF SLAM algorithm as they were implemented for this project. Some of these matrices may differ slightly from the way they are described in existing literature. For example, some matrices are often described to have a large number of zeros, guaranteeing that they only affect certain parts of the state when multiplied. Such matrix multiplications can often also be implemented to use smaller matrices without zeros, and the values can then be inserted in the correct part of a larger matrix.
