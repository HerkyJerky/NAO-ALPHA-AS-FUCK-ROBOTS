...
%\[ L_{p} > 120 + (L_{max}-120) \times \frac{L_{avg}}{L_{max}} \]
	\begin{equation}
	L_{p} > 120 + (L_{max}-120) \times \frac{L_{avg}}{L_{max}} \label{whiteImgProc}
	\end{equation}
Green:
	\begin{equation}
	G_{min} \leq H \leq G_{max} \label{greenImgProc}
	\end{equation}
Yellow:
	\begin{equation}
	Y_{min} \leq H \leq Y_{max} \label{yellowImgProc}
	\end{equation}
Where \\
	\begin{equation}
	L_{max} = maximumLuminance (usually 255) \label{LMAX}
	\end{equation}
	\begin{equation}
	L_{p} = luminanceOfPixel \label{LUM}
	\end{equation}
	\begin{equation}
	L_{avg} = prevApproximatedAverageLuminance 				\label{LAVG}
	\end{equation}
	\begin{equation}
	H = hueOfPixel\label{HUE}
	\end{equation}
and $G_{min}$, $G_{max}$, $Y_{min}$, $Y_{max}$ = lower and upper bounds for the hue of green and yellow.\\ \\
