...
{
public:
  Observer(boost::shared_ptr<AL::ALBroker> broker, const string &name);
  ~Observer();
  void setLogFileNamePrefix(const string &prefix);
  void init();
...
static const string sensor_names[];
static const string actuator_names[];
...
time_t current_time;
float hasgoal; // positive is true, negative is false
float goalx, goaly;
