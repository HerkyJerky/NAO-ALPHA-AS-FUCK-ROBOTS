...
Observer::Observer(boost::shared_ptr<AL::ALBroker> broker, const std::string &name) : AL::ALModule(broker, name), log_file_name_prefix("poop") {
  current_values_initialized = false;
  current_time = 0;

  setModuleDescription("observe in and outputs");

  functionName("setLogFileNamePrefix", getName(), "set log file name prefix");
  addParam("prefix", "the new log file name prefix");
  BIND_METHOD(Observer::setLogFileNamePrefix);

  functionName("start", getName(), "start observing");
  BIND_METHOD(Observer::start);

  functionName("stop", getName(), "stop observing");
  BIND_METHOD(Observer::stop);
}
...
void Observer::record_data(string filepath) {
  ...
  while (true) {
    ...
    stream << hasgoal << column_separator;
...
void Observer::init() {
  ...
  worldgoal_pointers[0] = (float*)memory.getDataPtr("Module/Observer/goal/x");
  worldgoal_pointers[1] = (float*)memory.getDataPtr("Module/Observer/goal/y");
  hasgoal_pointer = (float*)memory.getDataPtr("Module/Observer/hasgoal");
}

void Observer::setLogFileNamePrefix(const string &prefix) {
    log_file_name_prefix = prefix;
}

void Observer::start() {
  string filepath = "/home/nao/randomwalk/observations_" + log_file_name_prefix + "_" + boost::lexical_cast<string>(getpid());
